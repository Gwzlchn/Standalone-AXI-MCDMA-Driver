...
int check_two_array_in_ram(uint32_t *arr1, uint32_t *arr2, uint32_t arr_len) {
  int      err_cnt = 0;
  uint32_t i;
  for (i = 0; i < arr_len; i++) {
    if (!(arr1 + i) || !(arr2 + i)) {
    ...
    if (arr1[i] != arr2[i]) {
      err_cnt++;
      printf("data_src0[%d]:%08X, data_dst0[%d]:%08X\n", i, arr1[i], i, arr2[i]);
    }
  }
  return err_cnt;
}

void init_ram_multi_channel_buffer_data(void *   uram_init_data_base_addr,
